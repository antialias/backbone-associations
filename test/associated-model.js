...
var AssociatedModel = Associations.AssociatedModel;
var test = function (desc, expect, cb) {
    if (!cb) {
        cb = expect;
        expect = null;
    }
    return it(desc, function () {
        if (expect) assert.expect(expect);
        return cb.apply(this, arguments);
    })
};
test.only = function (desc, expect, cb) {
    if (!cb) {
        cb = expect;
        expect = null;
    }
    return it.only(desc, function () {
        if (expect) assert.expect(expect);
        return cb.apply(this, arguments);
    })
};
describe('associated model', function () {
    beforeEach(function () {
        global.Node = AssociatedModel.extend({
            relations:[
                {
    ...
    describe('AssociatedModel constructor', function () {
        var attr
        ...
        var map2dept;
        beforeEach(function () {
            Location = AssociatedModel.extend({
                defaults:{
                    id:-1,
                    add1:"",
                    add2:null,
            ...
            map2locs = function (ids) {
                if (!ids) { // unsure about this
                    ids = [];
                }
                if (ids.models) return ids; // assume ids is a collections if it has a models property
                if (isArray(ids) && ids.length > 0) {
                    if (isObject(ids[0])) //dummy logic to check whether array has ids or objects
                        return ids;
                } else {
                    if (isObject(ids))
                        return ids;
                ...
                ids = isArray(ids) ? ids.slice() : [ids];
                var result = [];
                forEach(ids, function (id) {
                    if (!id) return;
                    var mapped = find(locations.models, function (m) {
            ...
            Project = AssociatedModel.extend({
                relations:[
                    {
                        type:Associations.Many,
                        key:'locations',
            ...
            Department = AssociatedModel.extend({
                relations:[
                    {
                        type:Associations.Many,
                        key:'controls',
            ...
            Dependent = AssociatedModel.extend({
            ...
            //dept store
            store = new Collection([new Department({number:99, name:"sales"}), new Department({number:100, name:"admin"})]);

            map2dept = function (id) {

                if (id instanceof Department) return id;
                if (isObject(id)) return id;

                var found = find(store.models, function (m) {
                    return m.get('number') == id
                });
                return found ? found : id;
            };

            Employee = AssociatedModel.extend({
                relations:[
                    {
                        type:Associations.One,
                        key:'works_for',
        ...
        test("SEPARATOR", 13, function () {
            ...
            emp.get('works_for').on('change:locations[*]', function () {
                assert(true, "Fired emp.works_for change:locations[*]...");
            });

            emp.get('works_for').on('change:locations[0]->zip', function () {
                assert(true, "Fired emp.works_for change:locations[0].zip...");
            });

            emp.get('works_for').on('change:controls[0]->locations[0]->zip', function () {
                assert(true, "Fired emp.works_for change:controls[0]->locations[0]->zip...");
            });
        ...
        test("invalid relations", 4, function () {
            var em1 = AssociatedModel.extend({
                relations:[
                    {
                        type1:Associations.One, //no type specified
                        key:'parent',
                        relatedModel:'Node'
                    }
                ]
            ...
            try {
                ...
                equal(e.message === "type attribute must be specified and have the values Associations.One or Associations.Many", true)
            }

            var em2 = AssociatedModel.extend({
                relations:[
                    {
                        type:Associations.Oxne, //wrong value of type
                        key:'parent',
                        relatedModel:'Node'
                    }
                ]
            ...
            var em3 = AssociatedModel.extend({
                relations:[
                    {
                        type:Associations.One,
                        key:'parent',
            ...
            try {
                ...
                equal(e.message === "specify a relatedModel for Associations.One type", true)
            }

            var em4 = AssociatedModel.extend({
                relations: [
                    {
                        type: Associations.Many,
                        key: 'parent',
            ...
            try {
                ...
                equal(e.message === "type is of Backbone.Model. Specify derivatives of Associations.Collection", true)
            }
        });

        test("escape", 1, function () {
            emp.get('works_for').get("locations").at(0).set({'add1':'<a>New Address</a>'});
            equal(escape(emp.get('works_for').get("locations").at(0).get("add1")), '&lt;a&gt;New Address&lt;/a&gt;', "City should be in HTML-escaped version");
        });
        ...
        test("Set closure scope correctly - while setting BB Collection & Model instances directly", 5, function () {
            ...
            var Location = AssociatedModel.extend({
                defaults:{
                    name:"",
                    zip:""
                }
            ...
            var Group = AssociatedModel.extend({
                relations:[
                    {
                        type:Associations.One,
                        key:'meetup',
        ...
        test("Fetch collection repeatedly: Issue#47", 3, function () {
            ...
            var User = AssociatedModel.extend({
                defaults:{
                    name:"",
                    email:""
                }
            ...
            var Container = AssociatedModel.extend({
                relations:[
                    {
                        type:Associations.Many,
                        key:'xxx',
        ...
        test("Polymorphic Associate : Issue#23", 4, function () {
            ...
            Models.Job = AssociatedModel.extend({
                relations:[
                    {
                        type:Associations.One,
                        key:'organizable',
            ...
            Models.Comment = AssociatedModel.extend({
                relations:[
                    {
                        type:Associations.One,
                        key:'commentable',
            ...
            Models.Company = AssociatedModel.extend({

            });
            Models.Department = AssociatedModel.extend({

            });


            Models.Article = AssociatedModel.extend({

            });
            Models.StatusUpdate = AssociatedModel.extend({
        ...
        test("Fetch collection with reset: Issue#45", 5, function () {
            ...
            var Brand = AssociatedModel.extend();
            var Vendor = AssociatedModel.extend();
            var Tag = AssociatedModel.extend();

            var SearchFacet = AssociatedModel.extend({
                relations:[
                    {
                        type:Associations.Many,
                        key:'categories',
            ...
            var counter = 1;
            var SearchResult = AssociatedModel.extend({
                relations:[
                    {
                        type:Associations.One,
                        key:'searchFacet',
        ...
        test("IdAttribute: Issue#80", 2, function () {
            ...
            User = AssociatedModel.extend();

            Container = AssociatedModel.extend({
                relations: [
                    {
                        type: Associations.One,
                        key: 'owner',
        ...
        test("Global scope support: Issue#98", 3, function () {
            ...
            Associations.scopes.push(scope1);
            Associations.scopes.push(scope2);

            scope1.User = AssociatedModel.extend({
                defaults: {omid: "s1.u"}

            });

            scope2.User = AssociatedModel.extend({
                defaults: {omid: "s2.u"}
            });
        ...
        test("Remote Key: Issue#78", 11, function () {
            ...
            var Job = AssociatedModel.extend({
                relations: [
                    {
                        type: Associations.One,
                        key: 'location',
            ...
            var MultiContainer = AssociatedModel.extend({
                relations: [
                    {
                        type: Associations.Many,
                        key: 'owner',
            ...
            var UniContainer = AssociatedModel.extend({
                relations: [
                    {
                        type: Associations.One,
                        key: 'owner',
        ...
        test("Serialize subset of attributes - Issue#100", 2, function () {
            var Location = AssociatedModel.extend();
            var Contender = AssociatedModel.extend();
        ...
        test("Cycle save: Issue#51", 3, function () {
            ...
            Models.ChildMinder = AssociatedModel.extend({
            });

            Models.ChildrenMinders = Collection.extend({
                model:Models.ChildMinder
            });

            Models.Record = AssociatedModel.extend({
                relations:[
                    {
                        type:Associations.Many,
                        key:'childrenMinders',
        ...
        test("Polymorphic collection models: Issue#73", 2, function () {
            var Club = AssociatedModel.extend({
                relations:[{
                    type:Associations.Many,
                    key:'members',
                    relatedModel:function (relation, attributes) {
                        return function (attrs, options) {
                            if (isArray(attrs.dependents)) {
        ...
        //     var Fruit = AssociatedModel.extend();
        ...
        //         fids = isArray(fids) ? fids : [fids];
        //         type = type instanceof Collection ? type.model : type;
        ...
        //             fids,
        //             function (fid) {
        //                 return findWhere(store, {id:fid});
        //             }
        //         );
        //     };
        //
        //     var Oatmeal = AssociatedModel.extend({
        //         relations:[
        //             {
        //                 type:Associations.One,
        //                 key:'fruitable',
        ...
        //
        //
        //     //Test with Associations.Many
        //     var FruitExplosion = AssociatedModel.extend({
        //         relations:[
        //             {
        //                 type:Associations.Many,
        //                 key:'fruitable',
        ...
        //     equal(bananaExplosion.get('fruitable').at(0).get('species'), "Robusta");
        //     equal(bananaExplosion.get('fruitable').at(1).get('species'), "Yallaki");
        //
        //     var fruitableRef = bananaExplosion.get('fruitable');
        //     fruitableRef.add([5, 6]);
        //     equal(bananaExplosion.get('fruitable').at(2).get('species'), "Genetic Modification");
        //     equal(bananaExplosion.get('fruitable').at(3).get('species'), "Organic");
        //
        //
        //     var tomatoExplosion = new FruitExplosion({fruitable_type:Tomato, fruitable:[3, 4]});
        //     equal(tomatoExplosion.get('fruitable').at(0).get('species'), "Cherry");
        //     equal(tomatoExplosion.get('fruitable').at(1).get('species'), "Regular");
        //
        //
        // });
        ...
        test("Issue #28", 2, function () {
            ...
            var ItemModel = AssociatedModel.extend({
                relations:[
                    {
                        type:Associations.One,
                        key:'product',
                        relatedModel:AssociatedModel.extend()
                    }
                ],
                initialize:function () {
    ...
    //
    //         var ProductList = Collection.extend({
    //             model: Product
    //         });
    //
    //         var Product = AssociatedModel.extend({
    ...
    //     });
        ...
        test("Issue #133", 1, function () {
            ...
            var Foo = AssociatedModel.extend({});

            var Bar = AssociatedModel.extend({
                relations: [
                    {
                        type: Associations.One,
                        key: 'rel',
        ...
        test("Issue #121 - Traverse child upwards", 2, function () {
            ...
            var Designation = AssociatedModel.extend({
                initialize: function () {
        ...
        test("Issue #124", 7, function () {
            ...
            // TRH: commenting these out to get spec passing. Not sure if it's a good idea
            ...
            var ParentModel = AssociatedModel.extend({
                relations: [
                    {
                        type: Associations.One,
                        key: 'myChild',
            ...
            equal(parent.get('myChild') instanceof Model, true);
            equal(parent.get('myChild') instanceof AssociatedModel, false);
        ...
        test("Issue #109", 2, function () {
            ...
            var scope = {};
            var product = scope.Product = AssociatedModel.extend({});
            var products = scope.Products = Collection.extend({});

            var ItemModel = scope.ItemModel = AssociatedModel.extend({
                relations: [
                    {
                        type: Associations.Many,
                        key: 'products',
            ...
            ItemModel = scope.ItemModel = AssociatedModel.extend({
                relations: [
                    {
                        type: Associations.Many,
                        key: 'products',
        ...
        test('Issue #1177', 2, function() {
            ...
            var Baz = AssociatedModel.extend({
                relations: [
                    {
                        type: Associations.One,
                        key: 'fooRel',
        ...
        test("Issue  #35", 4, function () {
            ...
            var FieldInputType = AssociatedModel.extend({
                defaults:{
                    id:"-1",
                    type:"",
                    source:"<div/>"
            ...
            var Field = AssociatedModel.extend({
                relations:[
                    {
                        type:Associations.One,
                        key:'type',
        ...
        test("Issue #40", 5, function () {
            ...
            var CartItem = AssociatedModel.extend({
                defaults:{
                    qty:0
                }
            });

            var Cart = AssociatedModel.extend({
                relations:[
                    {
                        type:Associations.Many,
                        key:'items',
            ...
            var Account = AssociatedModel.extend({
                relations:[
                    {
                        type:Associations.One,
                        key:'cart',
        ...
        test("Issue #31", 5, function () {
            ...
            var Model1 = AssociatedModel.extend({});
            var Model2 = AssociatedModel.extend({
                relations:[
                    {
                        type:Associations.One,
                        key:"model1",
        ...
        test("Issue #31 nested collection", 2, function () {
            var Node = AssociatedModel.extend({
                defaults:{
                    id:null,
                    value:"",
                    nodes:[]
                ...
                relations:[
                    {
                        type:Associations.Many,
                        key:"nodes",
                        relatedModel:Associations.Self
                    }
                ]
            });
        ...
        test("parents - Issue #39", 3, function () {
            var Nested = AssociatedModel.extend();

            var Child = AssociatedModel.extend({
                relations:[
                    {
                        type:Associations.One,
                        key:'nested',
            ...
            var Root = AssociatedModel.extend({
                relations:[
                    {
                        type:Associations.One,
                        key:'child',
        ...
        test("Make collectionType a function as well - Issue #102", 1, function () {
            ...
            var Animal = AssociatedModel.extend({
                relations: [
                    {
                        type: Associations.One,
                        key: 'livesIn',
            ...
            var Zoo = AssociatedModel.extend({
                relations: [
                    {
                        type: Associations.Many,
                        key: 'animals',
        ...
        //     var associatedModel = {};
        //
        //     associatedModel.Zoo = AssociatedModel.extend({
        //         relations: [
        //             {
        //                 type: Associations.Many,
        //                 key: 'animals',
        ...
        //
        //     });
        //     associatedModel.Animal = AssociatedModel.extend({
        //         relations: [
        //             {
        //                 type: Associations.One,
        //                 key: 'livesIn',
        ...
        //     });
        //
        //     associatedModel.Country = AssociatedModel.extend({
        //         relations: [
        //             {
        //                 type: Associations.Many,
        //                 key: 'zoos',
        ...
        //
        //     var start = now();
        //     var c1 = new Collection(setupCounties(3, 100), {model: associatedModel.Country});
        ...
        //
        //     start = now();
        //     var c2 = new Collection(setupCounties(3, 100), {model: associatedModel.Country});
        ...
        //     });
        //
        //     var c1 = new Collection(setupCounties(1, 2), {model: associatedModel.Country});
        //
        //     var cb = function () {
        //         ok(true)
        //     };
        ...
        test("Many relation's options : parse", 3, function () {
            ...
            var Company = AssociatedModel.extend({
                url:"/company",
                relations:[
                    {
                        type:Associations.Many,
        ...
        test("One relation's options passed from parent", 2, function () {
            ...
            var Room = AssociatedModel.extend({
                url:"/unit",
                relations:[
                    {
                        type:Associations.One,
        ...
        test("Saving child of Model in Collection result in improper overwrite on return : issue#17", 2, function () {
            ...
            var Parent = AssociatedModel.extend({
                relations:[
                    {
                        type:Associations.One,
                        key:"child",
    ...
    describe("Cyclic Graph", function() {
        beforeEach(function () {
        ...
        test.only("Call toJSON in cycles -  Issue#58", function () {
    ...
    //
    //     test("Associations.One with child having falsy id", 1, function () {
    // var Child = AssociatedModel.extend({
    // });
    //
    // var Parent = AssociatedModel.extend({
    ...
    //         var listener = {};
    //         assign(listener, Backbone.Events);
    //
